...
You can download the file from: [https://github.com/dmcable/BIOSTAT620W26/blob/main/labs/lab10.qmd](https://github.com/dmcable/BIOSTAT620W26/blob/main/labs/lab10.qmd)
...
str(cv_states)
```
The dataset has 58,094 rows and 9 columns. We have a mix of character and integer/numeric variables in the dataset.
...
### COMPLETE CODE HERE
p1 <- ggplot(cv_states, aes(y=new_cases)) + geom_boxplot()
ggplotly(p1)

p2 <- ggplot(cv_states, aes(y=new_deaths)) + geom_boxplot()
ggplotly(p2)
...
ggplotly(p2)
```
Both of the new_cases and new_deaths columns have negative values, which is not possible unless they were correcting data. There are also some improbably high numbers, like a daily death count of over 3,000 people.
...
cv_states$newdeathsper100k <- as.numeric(format(round(cv_states$new_deaths/(cv_states$population/100000),5),nsmall=1))

# add a naive_CFR variable = deaths / cases
cv_states <- cv_states |> mutate(naive_CFR = round((deaths / cases),4))
...
ggplotly(p)
```
It's not a very strong trend, but an increase in population density appears to be associated with a slight increase in deaths per 100k.
...
plot_ly(
  ...
  text = ~paste(
    "State:", state,
    "<br>Date:", date,
    "<br>Naive CFR:", round(naive_CFR, 4)
  )
) |>
  layout(
    title = "Naive CFR Over Time by State",
...
cv_states |> 
  ...
  add_lines(
    x = ~date, 
    y = ~new_deaths * scale_factor,   # scale deaths
    name = paste0("New Deaths x", scale_factor),
    hoverinfo = "text",
    text = ~paste(
      "Date:", date,
      "<br>New Deaths:", new_deaths,
      "<br>Scaled Deaths:", round(new_deaths * scale_factor, 1)
    )
  ) |> 
  layout(
    title = "Florida: New Cases vs Scaled New Deaths Over Time",
    xaxis = list(title = "Date"),
    yaxis = list(title = "Count / Scaled Count"),
    hovermode = "compare"
  )
```
Using the hover feature, I looked at the case peak in early 2022 and found roughly a 1 month lag between peak cases and peak deaths.
...
plot_ly(x=colnames(cv_states_mat2), y=rownames(cv_states_mat2),
             ...
             showscale=TRUE)
```
In the raw case count heatmap, the high population states stick out, which is to be expected. California, Florida, New York, and Texas all have a distinctly higher case count around January 2022 than all other states. When we looked at the population-adjusted heatmap, we notice that fewer states stick out too much, but I noticed that Rhode Island had a particularly high COVID incidence in January 2022. After filtering the dates to only look every 2 weeks, I notice that Alaska, Louisiana, and Tennessee had periods of abnormally high case counts.
...
plotly::subplot(fig_pick.date, fig_Today, nrows = 2, margin = .05)
```
As we see in the maps, the case load has eased quite a bit since October 2021. There is almost no purple on today's map, indicating low case counts, while there are pockets of purple in the 2021 map. Incidence has clearly decreased over time.
